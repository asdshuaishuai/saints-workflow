...
## V3.1 · 霊台方寸 (Lingtai Fangcun - 現在)

- **自己省察**: 出力前に自己反省メカニズムを導入し、納品物の品質を保証。
- **動的ルーティング**: タスクの複雑度に基づき、最適な聖人を自動的にマッチング。
- **コンテキスト強化**: 長いコンテキストの管理を最適化し、正確な因果関係の追跡を実現。
- **アーキテクチャ最適化**: Claude 3.5/4 の特性に深く適応し、パフォーマンスを向上。

## V3.0 · 洪荒時代 (Great Primordial Era)

- **大陣システム**: 5つの主要な陣法（両儀、万仙、誅仙など）を段階に応じて回転。
- **先天法宝**: 開発ツールを強力な法宝（混元金斗など）に昇華。
- **封神榜**: 功徳、輪廻、因果のリストによる包括的なタスク追跡。
- **四聖会談**: 重要なアーキテクチャの決定における共同意思決定。
- **天道裁定**: デッドロック時の最終決定メカニズム。

---

## 概要

聖人ワークフローは、以下のフェーズからなる完全な開発プロセスを提供します：
...
### v3.1.0 · 霊台方寸 (2026-03-25)

- **自己省察**: 出力前の論理整合性チェックにより、コードとソリューションの品質を向上。
- **動的ルーティング**: タスクの意図解釈に基づくインテリジェントな聖人スケジューリング。
- **コンテキスト強化**: RAGと長いコンテキストの処理を最適化し、複雑なロジックを維持。
- **アーキテクチャ最適化**: プロフェッショナルな開発シナリオ向けにプロンプト戦略を刷新。

### v3.0.0 · 洪荒時代 (2026-03-20)

- **大陣システム**: 開発段階に応じた5つの主要陣法の導入。
- **先天法宝**: 開発ツールを強力なアーティファクトとして再定義。
- **封神榜**: 功徳、輪廻、因果リストによるタスク管理。
- **四聖会談**: 重要なタスクにおける複数エージェントの共同意思決定。
- **天道裁定**: 意思決定が困難な場合の最終裁定。
